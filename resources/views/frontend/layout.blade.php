...
<!-- Load class remove_unicode để sử dụng cho việc chuyển từ tiếng việt có dấu sang không dấu -->
<?php 
...
<body class="home">
    ...
    <div id="wrapper">
        ...
        <header>
            ...
            <script type="text/javascript">
                function search(){
                    ...
                    // Di chuyển đến trang tìm kiếm
                    location.href="{{ url('search') }}/"+key;
                }
            </script>
            <!-- ------------------------------------------------------- -->
            <div class="search">
                <form action="">
                    <input id="key" class="field" type="text" name="" placeholder="Search..." onkeypress="if (event.keyCode == 13) { search(); }">
                    <!-- event.keyCode == 13 là sự kiện javascript gõ phím enter(keycode = 13) -->
                    <!-- Có nghĩa khi nhập liệu thì bấm enter cũng giống như submit -->
                    <button onclick="search();" class="submit" type="button">
                </form>
        ...
        <?php 
            ...
            // limit(4) ->lấy 4 bản ghi
            $first_hot_news = DB::table("tbl_news")->where("c_hotnews","=","1")->orderBy("pk_news_id","desc")->offset(0)->limit(4)->get();
         ...
         <?php $n++ ?>
               <!-- first hot news -->
                <div class="cn_content" style="@if($n==1)top:0px;@endif background: url('{{ asset('upload/news/'.$rows->c_img) }}') no-repeat center #ffffff;background-size: 100% 100%;">
                    <style type="text/css">
                        .cn_content .caption p{padding: 0px;margin: 0px;}
                    </style>
                    <div class="caption">
            ...
            <div id="cn_list" class="cn_list">
                <div class="cn_page" style="display:block;">
                    <?php
                        $list_hot_news = DB::table("tbl_news")->where("c_hotnews","=","1")->orderBy("pk_news_id","desc")->offset(0)->limit(4)->get();
                     ?>
                     @foreach($list_hot_news as $rows)
                    <!-- list hot news -->
                    <div class="cn_item">
                        <div class="left-box">
